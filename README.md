...
بنابراین در کل در این بخش uvmcopy دیگه کپی فیزیکی نمیکند، صفحه را shared میکند، PTE_W رو خاموش و PTE_COW رو روشن میکند (هم در فرزند هم در والد)، و incref میزند. uvmunmap هم به‌ جای kfree مستقیم، حالا decref صدا میزند (چون صفحات ممکن است shared باشند).
